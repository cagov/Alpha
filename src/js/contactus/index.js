...
function loadResults() {
      ...
      if (loadCount < 50) {
        ...
        for (var i = 0; i < arrayLength; i++) {
          ...
          if (!(currentNavCharacter == previousNavCharacter)) {
            ...
            displayText += "<a name=\""+ currentNavCharacter.toLowerCase() +"\"><h3>" + currentNavCharacter +"</h3>";
            previousNavCharacter = currentNavCharacter;
          }
          else {
            if (recordCount < 1) {
              displayText += "<a name=\""+ previousNavCharacter.toLowerCase() +"\">&nbsp;</a><h3>" + previousNavCharacter +"</h3>";
              recordCount += 1;
            }
          }
  ...
  if((window.location.href.indexOf("contact-us/") > -1) && (window.location.href.indexOf("contact-us/results/") == -1) && (window.location.href.indexOf("contact-us/home/") == -1)) {
    buildAlphaAllList('agency-group','en','home/');
  }


  function buildResultsList(termToSearchFor, idOfDivToFill, lang, agencyLandingPage) {
    var alphaListingResults = [];
      if (lang === "") { lang = "en"; }
    ...
    if(window.location.href.indexOf("contact-us/results/") > -1) {
      var searchTerm = cleanInputItem(getQueryStringValue(window.location.href, 'searchitem'));
        ...
        if (loadFlag === false) {
          ...
          if (loadCount < 1000) {
            ...
            if (agencyArrayLength > 0) {
              document.getElementById("org-name-holder").innerHTML = this.apiData[r].FriendlyName


              document.title = "Agency Contact Information for " + this.apiData[r].FriendlyName + " - Alpha.CA.gov";
         
              document.getElementById("org-description-holder").innerHTML = this.apiData[r].Description
              var contactInfo ="";
              ...
              if((this.apiData[r].TwitterAccount)||(this.apiData[r].Facebook)||(this.apiData[r].YouTube)){
                ...
                document.getElementById("org-social-media").innerHTML = socialMediaInfo +"</div>";
              }
            }
          }
        }
